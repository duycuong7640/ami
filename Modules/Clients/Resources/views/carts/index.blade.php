        ...
        <div class="wrap-user-page">
            ...
            <div class="main-user-page">
                <form method="post" action="{{ route('client.card.store') }}" class="form-payment" id="form-cart">
                    ...
                    <div class="form-group">
                        <label for="address"><h3>Phương thức thanh toán</h3></label>
                        <div class="mb-3">
                            <label>
                                <input type="radio" name="pay_type" onclick="checkPayType(this.value)" value="1" checked="checked" />
                                Thanh toán sau khi nhận hàng
                            </label><br/>
                            <label>
                                <input type="radio" name="pay_type" onclick="checkPayType(this.value)" value="2" />
                                Thành toán chuyển khoản ngân hàng
                            </label>

                            <div class="mt-1 mb-4 ctct" id="content_pay" style="background: #ddd; display: none; padding: 15px; line-height: 22px;">
                                {!! $data_common['setting']->content_pay !!}
                            </div>
                        </div>
                    </div>
                    <div class="wrap-action-cart">
...
@section('scripts')
    ...
    <script type="text/javascript">
        function checkPayType(pay) {
            if (parseInt(pay) == parseInt(1)) {
                $("#content_pay").hide();
            }else{
                $("#content_pay").show();
            }
