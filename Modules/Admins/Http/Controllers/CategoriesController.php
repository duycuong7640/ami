...
class CategoriesController extends Controller
{
    ...
    public function __construct(CategoryService $categoryService, AllCacheService $allCacheService)
    {
    ...
    public function index()
    {
        try {
            ...
            $data['multi'] = $this->categoryService->getListMenu(['multi' => true]);
    ...
    public function update(EditRequest $request, $id)
    {
        try {
            ...
            if (empty($data['detail']->id)) return abort(404);
            $_params = $request->all();
            if ($this->categoryService->update($_params, $id, $data['detail'])) {
                $this->createCache();
