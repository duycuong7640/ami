...
class PostfourthsController extends Controller
{
    ...
    public function __construct(CategoryService $categoryService, PostService $postService, AllCacheService $allCacheService)
    {
        ...
        $this->type = [$categoryService::TYPE[3]];
    }
    ...
    public function index()
    {
        try {
            ...
            return view('admins::postthrees.index', ['data' => $data]);
    ...
    public function create()
    {
        try {
            ...
            return view('admins::postthrees.create', ['data' => $data]);
        } catch (\Exception $e) {
            abort('500');
        }
    }
    ...
    public function store(CreateRequest $request)
    {
        try {
            ...
            if ($this->postService->create($_params)) {
                //$this->createCache();
                return redirect(route('admin.postthree.index'));
            } else {
    ...
    public function edit($id)
    {
        try {
            ...
            return view('admins::postthrees.edit', ['data' => $data]);
        } catch (\Exception $e) {
    ...
    public function update(EditRequest $request, $id)
    {
        try {
            ...
            if ($this->postService->update($_params, $id)) {
                ...
                return redirect(route('admin.postthree.index', ['page' => !empty($_GET['page']) ? $_GET['page'] : 1]));
            } else {
                $errors = new MessageBag(['error' => __('admins::layer.notify.fail')]);
                return back()->withInput($_params)->withErrors($errors);
            }
