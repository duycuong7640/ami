...
class PosteventsController extends Controller
{
    ...
    public function __construct(CategoryService $categoryService, PostService $postService, AllCacheService $allCacheService)
    {
        ...
        $this->type = [$categoryService::TYPE[7]];
    }
...
//            }
    ...
    public function create()
    {
        try {
            ...
            return view('admins::postevents.create', ['data' => $data]);
        } catch (\Exception $e) {
            abort('500');
        }
    }
    ...
    public function store(CreateRequest $request)
    {
        try {
            ...
            if ($this->postService->create($_params)) {
                //$this->createCache();
                return redirect(route('admin.pevent.index'));
            } else {
    ...
    public function edit($id)
    {
        try {
            ...
            return view('admins::postevents.edit', ['data' => $data]);
        } catch (\Exception $e) {
    ...
    public function update(EditRequest $request, $id)
    {
        try {
            ...
            if ($this->postService->update($_params, $id)) {
                ...
                return redirect(route('admin.pevent.index', ['page' => !empty($_GET['page']) ? $_GET['page'] : 1]));
            } else {
                $errors = new MessageBag(['error' => __('admins::layer.notify.fail')]);
                return back()->withInput($_params)->withErrors($errors);
            }
