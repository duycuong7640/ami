...
class FixfourthsController extends Controller
{
    ...
    public function __construct(AdvertisementService $advertisementService)
    {
        ...
        $this->type = $this->advertisementService::TYPE[8];
    }
    ...
    {
        try {
            $data['common'] = Helpers::titleAction([__('admins::layer.common.index.title'), __('admins::layer.common.index.title2')]);
            $data['list'] = $this->advertisementService->getList(['limit' => 10, 'type' => $this->type]);
            return view('admins::fixfourths.index', ['data' => $data]);
        } catch (\Exception $e) {
            abort('500');
        }
    }
    ...
    {
        try {
            $data['common'] = Helpers::titleAction([__('admins::layer.common.add.title'), __('admins::layer.common.index.title2')]);
            return view('admins::fixfourths.create', ['data' => $data]);
        } catch (\Exception $e) {
            abort('500');
        }
    }
    ...
    public function store(CreateRequest $request)
    {
        try {
            ...
            if ($this->advertisementService->create($_params)) {
                return redirect(route('admin.fixfourth.index'));
            } else {
    ...
    {
        try {
            $data['common'] = Helpers::titleAction([__('admins::layer.common.edit.title'), __('admins::layer.common.index.title2')]);
            $data['detail'] = $this->advertisementService->findById($id);
            if (empty($data['detail']->id)) return abort(404);
            return view('admins::fixfourths.edit', ['data' => $data]);
        } catch (\Exception $e) {
    ...
    public function update(EditRequest $request, $id)
    {
        try {
            ...
            if ($this->advertisementService->update($_params, $id)) {
                ...
                return redirect(route('admin.fixfourth.index', ['page' => (!empty($_GET['page']) ? $_GET['page'] : 1)]));
            } else {
                $errors = new MessageBag(['error' => __('admins::layer.notify.fail')]);
                return back()->withInput($_params)->withErrors($errors);
            }
