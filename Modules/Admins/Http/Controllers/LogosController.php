...
class LogosController extends Controller
{
    ...
    public function __construct(AdvertisementService $advertisementService)
    {
        ...
        $this->type = $this->advertisementService::TYPE[1];
    }
    ...
    public function index()
    {
        try {
            ...
            $data['list'] = $this->advertisementService->getList(['limit' => 10, 'type' => array('logo','favico_web','favico_admin','voucher')]);
            return view('admins::logos.index', ['data' => $data]);
    ...
    {
        try {
            $data['common'] = Helpers::titleAction([__('admins::layer.advertisement.add.title'), __('admins::layer.advertisement.index.title2')]);
            return view('admins::logos.create', ['data' => $data]);
        } catch (\Exception $e) {
            abort('500');
        }
    }
    ...
    {
        try {
            $data['common'] = Helpers::titleAction([__('admins::layer.advertisement.edit.title'), __('admins::layer.advertisement.index.title2')]);
            $data['detail'] = $this->advertisementService->findById($id);
            if (empty($data['detail']->id)) return abort(404);
            return view('admins::logos.edit', ['data' => $data]);
        } catch (\Exception $e) {
    ...
    public function update(EditRequest $request, $id)
    {
        try {
            ...
            if ($this->advertisementService->update($_params, $id)) {
                ...
                return redirect(route('admin.logo.index', ['page' => (!empty($_GET['page']) ? $_GET['page'] : 1)]));
            } else {
                $errors = new MessageBag(['error' => __('admins::layer.notify.fail')]);
                return back()->withInput($_params)->withErrors($errors);
            }
