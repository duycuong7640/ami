                            ...
                            <div class="input-group mb-0 mr-sm-2">
                                <select name="type" class="form-control form-select-search">
                                    ...
                                    @foreach($type_redirect as $row)
                                        <option @if(isset($_GET['type'])) @if($_GET['type'] == $row) selected @endif @endif value="{{ $row }}">{{ $row }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <button type="submit"
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a class="icon-form" title="edit"
                                               href="{{ route('admin.redirect.edit', ['id' => $row->id, 'page' => $data['list']->currentPage()]) }}">
                                                <i class="icon-note"></i>
                                            </a>
                                            <a class="icon-form status active"
                                               href="{{ route('admin.redirect.status', ['id' => $row->id, 'field' => 'status']) }}">
                                                {!! (($row->status == 1) ? '<i class="icon-check"></i>' : '<i class="icon-close"></i>') !!}
                                            </a>
                                            <a class="icon-form"
                                               href="javascript:confirmDelete('{{ route('admin.redirect.destroy', ['id' => $row->id]) }}','@lang('admins::layer.notify.confirm.delete')')">
