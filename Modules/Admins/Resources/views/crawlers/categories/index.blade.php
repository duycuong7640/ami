...
{{--                                    <th width="50">@lang('admins::layer.table.id')</th>--}}
                                ...
                                @foreach($data['list'] as $k=>$row)
                                    <tr>
                                        ...
                                        <td style="width: 250px; word-break: break-word; white-space: initial;">{{ $row->url }} <a href="{{ $row->url }}" target="_blank">Link</a></td>
...
{{--                                        <td>{{ $row->id }}</td>--}}
                                        <td>
                                            <a class="icon-form" title="edit" href="{{ route('admin.crawler.category.edit', ['crawler_website_id' => request('crawler_website_id'), 'id' => $row->id, 'page' => $data['list']->currentPage()]) }}">
                                                <i class="icon-note"></i>
                                            </a>
                                            <a class="icon-form status active" href="{{ route('admin.crawler.category.status', ['crawler_website_id' => request('crawler_website_id'), 'id' => $row->id, 'field' => 'status']) }}">
