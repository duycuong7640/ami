                ...
                <div class="d-flex flex-column flex-md-row align-items-center">
                    ...
                    <div class="wrapper ml-md-auto d-flex flex-column flex-md-row kanban-toolbar ml-n2 ml-md-0 mt-4 mt-md-0">
                        <div class="d-flex mt-4 mt-md-0">
                            <a href="{{ route('admin.fixtwo.create') }}">
                            ...
                            <table class="table table-striped table-bordered table-hover">
                                ...
                                @foreach($data['list'] as $k=>$row)
                                    <tr>
                                        ...
                                        <td>{{ $row->id }}</td>
                                        <td>
                                            <a class="icon-form" title="edit" href="{{ route('admin.fixtwo.edit', ['id' => $row->id, 'page' => $data['list']->currentPage()]) }}">
                                                <i class="icon-note"></i>
                                            </a>
                                            <a class="icon-form status active" href="{{ route('admin.fixtwo.status', ['id' => $row->id, 'status' => (($row->status == 1) ? 0 : 1)]) }}">
                                                {!! (($row->status == 1) ? '<i class="icon-check"></i>' : '<i class="icon-close"></i>') !!}
                                            </a>
                                            <a class="icon-form" href="javascript:confirmDelete('{{ route('admin.fixtwo.destroy', ['id' => $row->id]) }}','@lang('admins::layer.notify.confirm.delete')')">
