                    ...
                    <div class="d-flex flex-column flex-md-row align-items-center">
                        ...
                        <div class="wrapper ml-md-auto d-flex flex-column flex-md-row kanban-toolbar ml-n2 ml-md-0 mt-4 mt-md-0">
                            <div class="d-flex mt-4 mt-md-0">
                                ...
                                <a href="{{ route('admin.pevent.index') }}">
                    ...
                    <div class="card">
                        <div class="card-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div style="overflow: hidden; clear: both;">
                                            <select name="d" class="form-control" style="width: 80px; float: left;">
                                                ...
                                                @for($i=1; $i<=31; $i++)
                                                    <option @if(!empty($data['detail']->dienra)) @if(date('d', strtotime($data['detail']->dienra)) == $i) selected @endif @endif value="{{$i}}">{{$i}}</option>
                                                @endfor
                                            </select>
                                            <select name="m" class="form-control" style="width: 85px; float: left; margin: 0 10px;">
                                                <option value="">Tháng</option>
                                                @for($i=1; $i<=12; $i++)
                                                    <option @if(!empty($data['detail']->dienra)) @if(date('m', strtotime($data['detail']->dienra)) == $i) selected @endif @endif value="{{$i}}">{{$i}}</option>
                                                @endfor
                                            </select>
                                            <select name="y" class="form-control" style="width: 80px; float: left;">
                                                <option value="">Năm</option>
                                                @for($i=2000; $i<=2100; $i++)
                                                    <option @if(!empty($data['detail']->dienra)) @if(date('Y', strtotime($data['detail']->dienra)) == $i) selected @endif @endif value="{{$i}}">{{$i}}</option>
                                                @endfor
                                            </select>
                                            <select name="h" class="form-control" style="width: 80px; float: left; margin: 0 10px;">
                                                <option value="">Giờ</option>
                                                @for($i=0; $i<=23; $i++)
                                                    <option @if(!empty($data['detail']->dienra)) @if(date('H', strtotime($data['detail']->dienra)) == $i) selected @endif @endif value="{{$i}}">{{$i}}</option>
                                                @endfor
                                            </select>
                                            <select name="i" class="form-control" style="width: 80px; float: left;">
                                                <option value="">Phút</option>
                                                @for($i=0; $i<=59; $i++)
                                                    <option @if(!empty($data['detail']->dienra)) @if(date('i', strtotime($data['detail']->dienra)) == $i) selected @endif @endif value="{{$i}}">{{$i}}</option>
                                                @endfor
                                            </select>
                                            <select name="s" class="form-control" style="width: 80px; float: left; margin: 0 10px;">
                                                <option value="">Giây</option>
                                                @for($i=0; $i<=59; $i++)
                                                    <option @if(!empty($data['detail']->dienra)) @if(date('s', strtotime($data['detail']->dienra)) == $i) selected @endif @endif value="{{$i}}">{{$i}}</option>
                                                @endfor
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div id="tab_en" class="tabHide" style="display: none;">
                                ...
                                <div style="padding: 15px; background: #ddd; margin-bottom: 15px;">
                                    <div class="form-group">
                                        <label>Vị trí</label>
                ...
                <div class="col-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                <label>Meta des</label>
                                <textarea type="text" name="meta_des_en" class="form-control"
                                          placeholder="">{{ $data['detail']->meta_des_en }}</textarea>
                            </div>
                            <div class="form-group">
                                <label>Meta key</label>
                                <textarea type="text" name="meta_key_en" class="form-control"
