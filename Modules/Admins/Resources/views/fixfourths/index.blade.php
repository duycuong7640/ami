        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="d-flex flex-column flex-md-row align-items-center">
                    <h4 class="mb-md-0 mb-4 mr-4" style="display: none !important;">{{ !empty($data['common']['title']) ? $data['common']['title'] : '' }}</h4>
                    <div class="wrapper d-flex align-items-center" style="display: none !important;">
                    ...
                    @if(!count($data['list']))
                        <div class="wrapper ml-md-auto d-flex flex-column flex-md-row kanban-toolbar ml-n2 ml-md-0 mt-4 mt-md-0">
                            <div class="d-flex mt-4 mt-md-0">
                                <a href="{{ route('admin.fixfourth.create') }}">
                                ...
                                @foreach($data['list'] as $k=>$row)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <hr/>
                                            <img src="{{ asset($row->thumbnail2) }}" class="mw-100">
                                        </td>
                                        <td>{{ $row->title }}</td>
                                        ...
                                        <td>
                                            <a class="icon-form" title="edit"
                                               href="{{ route('admin.fixfourth.edit', ['id' => $row->id, 'page' => $data['list']->currentPage()]) }}">
                                                <i class="icon-note"></i>
                                            </a>
                                            <a class="icon-form status active"
                                               href="{{ route('admin.fixfourth.status', ['id' => $row->id, 'status' => (($row->status == 1) ? 0 : 1)]) }}">
                                                {!! (($row->status == 1) ? '<i class="icon-check"></i>' : '<i class="icon-close"></i>') !!}
                                            </a>
                                            <a class="icon-form"
                                               href="javascript:confirmDelete('{{ route('admin.fixfourth.destroy', ['id' => $row->id]) }}','@lang('admins::layer.notify.confirm.delete')')">
