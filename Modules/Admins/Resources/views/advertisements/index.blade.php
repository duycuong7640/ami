...
{{--                                        <td>{{ $row->title }}</td>--}}
                                        ...
                                        <td>{{ $row->id }}</td>
                                        <td>
                                            <a class="icon-form" title="edit" href="{{ route('admin.advertisement.edit', ['id' => $row->id, 'page' => $data['list']->currentPage()]) }}">
                                                <i class="icon-note"></i>
                                            </a>
                                            <a class="icon-form status active" href="{{ route('admin.advertisement.status', ['id' => $row->id, 'status' => (($row->status == 1) ? 0 : 1)]) }}">
                                                {!! (($row->status == 1) ? '<i class="icon-check"></i>' : '<i class="icon-close"></i>') !!}
                                            </a>
                                            <a class="icon-form" href="javascript:confirmDelete('{{ route('admin.advertisement.destroy', ['id' => $row->id]) }}','@lang('admins::layer.notify.confirm.delete')')">
